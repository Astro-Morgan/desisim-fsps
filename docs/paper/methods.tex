...
L_{\rm Edd}$) holds to within $\sim$10\%; and the full synthesized spectral
shape and absolute normalization together reproduce
\citet{KubotaDone2018}'s own published Figure~9 ($M=10^8\,M_\odot$,
$\dot m=0.05$ and $0.5$, evaluated as a flux at their stated fiducial
100~Mpc distance and $i=45^\circ$ inclination) to within $\sim$1\% once
compared on genuinely equal footing. One precisely-diagnosed bug was found
in the official Fortran's own diagnostic print statements along the way (a
variable, already correctly repurposed as a flux for the final output
spectrum's own normalization, gets reused a few lines later in a
temperature print statement that expects a true luminosity instead) --
confirmed by exactly reproducing the Fortran's printed value when the same
variable reuse is deliberately replicated; it does not affect the model's
actual physics or this implementation, which always uses the correct,
undivided luminosity.
